...
const Machines = () => {
  ...
  useEffect(() => {
    const ws = new WebSocket('ws://localhost:8000/ws/socket-server/dady/');
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (data.data) {
          ...
          console.error("Unexpected data format from WebSocket:", event.data);
        }
      } catch (jsonError) {
        ...
        <div className="bg-white">
          ...
          <div className="flex gap-5 p-5">
            <div className="bg-[#F4F7FF] rounded-md text-gray-400 p-5 flex justify-between w-screen">
              ...
              <p className="text-xl font-urbanist font-semibold">Machine Type</p>
              <p className="text-xl font-urbanist font-semibold">Temperature</p>
              <p className="text-xl font-urbanist font-semibold">Vibration Level or speed</p>
              <p className="text-xl font-urbanist font-semibold"></p>
               </div>
               </div>

          <div className="flex flex-col">
            {machines.map((machine) => (
              <Machine key={machine.id} machineData={machine} />
            ))}
          </div>
        </div>
      </div>
